...
import BarChart from "@/components/animata/bar-chart";
import AvatarList from "@/components/animata/avatar-list";
import Report from "@/components/animata/report";
import WideCard from "@/components/animata/wide-card";
import Counter from "@/components/animata/counter";
import Ticker from "@/components/animata/ticker";
import TypingText from "@/components/animata/typing-text";
...
function BoldCopy({
    ...
    return (
        ...
        >
            <div
                className={cn(
                    "text-4xl font-black  text-foreground/15 transition-all group-hover:opacity-50 md:text-8xl",
                    backgroundTextClassName,
                )}
            >
                {text}
            ...
            <div
                className={cn(
                    "text-md absolute font-black  text-foreground transition-all group-hover:text-4xl md:text-3xl group-hover:md:text-8xl",
                    textClassName,
                )}
            >
                {text}
    ...
    return (
        <BentoCard className="flex flex-col bg-orange-300">
            ...
            <div className="mt-auto">
                <div className="text-sm font-medium">Is this fare feasible?</div>
                <div className="font-semibold">
                    <TypingText text="Yes, it's cheaper tho." waitTime={2000} alwaysVisibleCount={0} />
                </div>
            </div>
        </BentoCard>
    );
...
function FeatureFive() {
    return (
        <BentoCard className="flex flex-col items-center justify-center bg-zinc-300 sm:col-span-2">
            <BoldCopy text="RideX" className="bg-transparent" textClassName="text-zinc-800" />
        </BentoCard>
    );
}
...
function FeatureEight() {
    return (
        <BentoCard className="relative flex flex-col bg-blue-200 sm:col-span-2">
            <WideCard />
            <div className="mt-4">
                <div className="text-lg font-black text-blue-800">Real time notifications</div>
                <p className="text-sm">Our real time messages, notifications and tracking has got you covered.</p>
            </div>
        </BentoCard>
    );
}
...
export default function Eight() {
    return (
        <div className="storybook-fix max-w-7xl justify-self-center p-20">
